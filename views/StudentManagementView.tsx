  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg p-6 dark:bg-slate-800">
        ...
        {isLoading ? (
          <p className="dark:text-gray-300">Loading user list...</p>
        ) : users.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-600 text-lg dark:text-gray-400">No student or teacher accounts have been registered yet.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
              ...
              <tbody className="bg-white divide-y divide-gray-200 dark:bg-slate-800 dark:divide-slate-700">
                {filteredUsers.length > 0 ? (
                  ...
                  <tr>
                      <td colSpan={profile?.role === 'admin' ? 6 : 5} className="text-center py-8 text-gray-500 dark:text-gray-400">
                          {profile?.role === 'teacher' ? 'No students have been registered yet.' : 'No users match your search criteria.'}
                      </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-lg shadow-2xl max-w-md w-full mx-4 dark:bg-slate-800">
                ...
                <div className="flex justify-end space-x-4 mt-6">
                    ...
                    <button onClick={handleDeleteUser} className="bg-red-600 hover:bg-red-700 text-white font-semibold px-6 py-2 rounded-lg">Delete User</button>
                </div>
            </div>
        </div>
      )}
