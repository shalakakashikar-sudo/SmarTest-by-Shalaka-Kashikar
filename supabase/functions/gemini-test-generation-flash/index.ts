// supabase/functions/gemini-test-generation-flash/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Generate a test for a '${difficulty}' level on the topic of "${topic}".
      Create exactly ${numQuestions} questions.
      The question types MUST be from this list: ${questionTypes.join(', ')}.`;
    
    const jsonResponseString = await generateWithFallback(prompt, testSchema, 'gemini-flash-latest');

    return new Response(jsonResponseString, {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error in gemini-test-generation-flash:', error);
