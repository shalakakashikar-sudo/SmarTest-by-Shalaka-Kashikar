// supabase/functions/get-my-results/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Create admin client to fetch data, bypassing RLS.
    const adminClient = createClient(supabaseUrl, serviceRoleKey, {
      auth: { autoRefreshToken: false, persistSession: false }
    });
    
    // 3. Fetch all test results for the authenticated user.
    ...
    console.error('Error in get-my-results function:', error.message);
